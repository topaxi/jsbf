var Brainfuck = (function(undefined){
	...
	Brainfuck.prototype = {
		...
		_generateJSSource:function(code, optimized){
			var codeLength     = code.length,
			    ...
			    JSSource = 'var index = 0,\n    mem = [' + (function(maxArrayLength){
			    	var s = [];
		...
		_interpretOptimizedInstruction:function(instruction, count){
			switch(instruction){
				case '+': return 'mem[index] += '+ count +';\n';
				case '-': return 'mem[index] -= '+ count +';\n';
				case '<': return 'index -= '+ count +';\n';
				case '>': return 'index += '+ count +';\n';
				case '0': return 'mem[index] = 0;\n';
				default : return '';
			}
		},
		_interpretInstruction:function(instruction){
			switch(instruction){
				case '+': return '++mem[index];\n';
				case '-': return '--mem[index];\n';
				case '<': return '--index;\n';
				case '>': return '++index;\n';
				case '.': return 'output += String.fromCharCode(mem[index]);\n';
				case '[': return 'while(mem[index]){\n';
				case ']': return '}\n';
				case ',':
